...
class MilestoneSixPagedAttentionPublicationTests(unittest.TestCase):
    @classmethod
    ...
    def test_signed_index_and_compatibility_cover_all_winners(self) -> None:
        index = self.validate_signed("pack-index", f"{OPERATOR}-1.0.json", "pack-index")
        ...
        self.assertEqual(len({pack["pack_id"] for pack in index["packs"]}), EXPECTED_COUNT)
        self.assertEqual(
    ...
    def test_dispatch_evidence_approval_and_index_are_linked(self) -> None:
        ...
        for entry in dispatch["entries"]:
            ...
            self.assertEqual(approval["reviewer"], "zmu")
            self.assertEqual(
                entry["winner"],
                {
                    key: artifact[key]
